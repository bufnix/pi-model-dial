...
The editor border follows the active mode's color rather than the thinking level, and its top-right corner shows the mode's gauge glyph (`󰡵 medium`) plus, when [`pi-autonomy-slider`](https://github.com/bufnix/pi-autonomy-slider) is loaded, the autonomy glyph and level (`󰚩 default`).
